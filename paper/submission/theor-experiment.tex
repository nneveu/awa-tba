...
\begin{table}%[h!]
	\begin{center}
		...
		\label{extable}
		\begin{tabular*}{0.48\textwidth}{l c|c|c}
			& Gene Mutation \qquad& Recombination \qquad& Number of completed\vspace{-0.75em}\\
			& Probability \qquad& Probability \qquad&  generations \\
			\midrule
			ex-1 &  0.1  & 0.9  &  96 \\
			ex-2 &  0.3  & 0.7  &  81 \\
			ex-3 &  0.8  & 0.2  &  53 \\
...
The OPAL input file for these cases and all subsequent optimization runs in this paper, 
can be found at in following repository: \url{https://github.com/nneveu/awa-tba}.
...
A Power Extractor and Transfer Structure (PETS) will be located after the septum and quadrupoles in Fig.~\ref{awa-tba}.
This device is putting the tightest constraints on the beam parameters.  
The aperture diameter is \SI{17.6}{mm}, and bunch length should be below \SI{2}{mm}
...
the max beamsizes at the PETS location were below \SI{10}{mm} with a bunch length of $rms_z=\SI{1.6}{mm}$. 
...
the extracted beam sizes at different kicker angles compared to simulation~\cite{neveuthesis}.  
