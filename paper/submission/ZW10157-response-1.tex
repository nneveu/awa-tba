...
machine configurations can be done at different charge levels. While other codes such as GPT [updated ref here] and ELEGANT [updated ref here], also have integrated genetic optimization algorithms; the OPAL framework, differentiates itself by being open source (i.e. free to use), massively parallel, and fully 3D.
}
...
empirical tuning by operators a common method used at many research facilities.
...
{\bf Our response:} {\color{blue} New references were added, as mentioned above.}
...
{\bf Our response:} {\color{blue} A line has been here added indicating this capability in GPT too.}
...
{\bf Our response:} {\color{red} The AWA is a high charge charge accelerator. The strong non-linearities
justifying the full 3D approach. At the same time AWA is a facility where many different accelerator research experiments take
place. Therefore it is very desirable, and was one of the important research objectives, to come up with a precise, e.g. 3D
model embedded into a multi-objective optimisation framework. The OPAL framework, differentiates itself, being massively paralle,
full 3D and, and simple to use i.e.\ user friendly.}
...
{\bf Our response:} {\color{blue} We agree that some references of import were missing and should have been included.
	...
	We thank the reviewer for the list of references. 
	References described or referenced by A. Hofler in the above were excluded from 
	the bibliography. 
}
...
{\color{blue}{Included in updated bib:}}
...
{\color{blue}{Exluded from bib, because  they are in A. Hofler 13':}}
...
{\bf Our response:} {\color{red} The statements on the lines  17-19 of our initial submission are misleading, as
the referee correctly recognised. We replace them with the following:  

Despite the great success of on-line models, and various model dependent and model independent tuning and 
optimisation techniques, in research facilities such as AWA empirical tuning is still one of the methods used. Due to the 
non-linear problem mainly from space charge, simple and fast models are applicable only in a very 
restricted manner. When it comes to multi-objective optimisation then the situation is even more complicated.
}
...
{\bf Our response: } {\color{red} This section has being shortened. However we would like to 
leave the existing description, in order to drive home the fact that the framework can be 
easily extended with other optimisation algorithms. Something that you can not find in most of the 
existing codes.}`
