...
\author{N. Neveu\footnotemark[7]\ \footnotemark[8]
  \and Y. Ineichen\footnotemark[2]\ \footnotemark[3]\ \footnotemark[4]
  \and A. Adelmann\footnotemark[2]\ \footnotemark[6]
  \and A. Kolano\footnotemark[2]\ \footnotemark[5]
  \and C. Bekas\footnotemark[3]
  \and A. Curioni\footnotemark[3]
  \and P. Arbenz\footnotemark[4]
}

\maketitle

\renewcommand{\thefootnote}{\fnsymbol{footnote}}
\footnotetext[2]{PSI, Villigen, Switzerland}
\footnotetext[3]{IBM Research -- Zurich, Switzerland}
\footnotetext[4]{Department of Computer Science, ETH Zurich, Switzerland}
\footnotetext[5]{University of Huddersfield, West Yorkshire, United Kingdom}
\footnotetext[1]{Illinois Institute of Technology, Chicago, USA}
\footnotetext[8]{Argonne National Laboratory, Lemont, USA}
\footnotetext[6]{Corresponding author: \url{andreas.adelmann@psi.ch}}
...
\begin{keywords}
multi-objective optimization, beam dynamics, high performance computing
\end{keywords}
...
\begin{figure}[h!]
\begin{center}\normalsize
  \begin{tikzpicture}[text=black]%[scale=0.95, transform shape]
...
Here, we will focus on a real world application arising from PSI's
  \textsc{SwissFEL}~\cite{pedr:10} activities, which have an immediate and a
  long-range impact, by optimizing important parameters of PSI's \textsc{250
  MeV injector} test facility (and later for the full PSI \textsc{SwissFEL}).
...
A validation and a proof of concept application in two beam dynamics problems is
...
%
\noindent
  we implemented an expression parser using Boost
  Spirit\footnote{\url{http://boost-spirit.com/}}.
...
The function arguments specified in the corresponding expression are stored in
  a \texttt{std::vector} of Boost
  variants\footnote{\url{www.boost.org/doc/html/variant.html}} that can be
...
As shown in Listing 5 this is done by creating a collection of Boost
  functions\footnote{\url{www.boost.org/libs/function/}} corresponding to the
...
\begin{align} \label{eqn:bench}
  \text{ min} \quad & \quad \left[ 1 - \exp \left( -1 \left(
...
The authors thank the \textsc{SwissFEL} and AWA teams for contributing to the
  formulation of optimization problems. We gratefully acknowledge the computing resources provided on Bebop, a high-performance computing cluster operated by the Laboratory Computing Resource Center at Argonne National Laboratory.
\appendix
