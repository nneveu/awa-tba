...
recombination probability, number of individuals (initial and independent), and number of generations. 
Given the beam line and design variables given in Section \ref{awaproblem}, 
four optimization experiments were done with varying NSGA-II parameters, see Table \ref{extable}. 
\begin{table}%[h!]
	\begin{center}
...
and the number of independent individuals per generation was fixed at 328. 
...
We also reduced the number of objectives to the four most important for quicker evaluation of these tests.
These objectives include: 
$\epsilon_{x, s_1}\text{, } \epsilon_{x, s_2}\text{, } \text{rms}_{s, s_1}\text{, and }  \text{rms}_{s, s_2}$
The optimization in the OPAL input file was defined as follows:
\vspace{0.2cm}
{\footnotesize \begin{verbatim}
...
They were each allowed to run for twenty four hours. 
Similar to the time step scan, the goal of this exercise was to determine which set of optimization
parameters strongly influence the results, and whether there was a time to solution
difference.



After collection of all data, the Pareto front of each data set was plotted.
