...
The optimization location is chosen as the entrance to the fifth quadrupole, $s_3$, on the dog leg.
Minimizing beam sizes here will enable capture and further focus with quadrupoles five and six before space charge effects dominate the beam. 
...
\begin{figure}
	...
	\node[fill=white, inner sep=2pt] (txt2) at (0,20) {Gun};
	\node[fill=white, inner sep=2pt] (txt2) at (20,20) {Accelerating Cavities};
	\node[fill=white, inner sep=2pt] (txt2) at (88,28) {Quadrupoles};
	\draw [blue, ->, line width=1] (99,28) -- (99, 21);
...
$s_3=19.4$~[m], at the entrance of the fifth quadrupole in Fig.~\ref{awa-tba}.  
...
Before running a full scale optimization of the problem described in \ref{awaproblem}, 
...
In the drifts and linac tanks, \num{1E-11} was sufficient, 
but still not acceptable near the quadrupoles. 

in combination with adjusting the time step (dT) near sensitive elements 
(i.e. quadurpoles, kicker, septum). 
...
\begin{table}%[h!]
	\begin{center}
		\caption{Checkmarks (\cmark) indicate desired physics is resolved. 
			An (\xmark) indicates the time step is too large.}
		\label{timestep}
...
\subsubsection{AWA Optimization Problem}
\begin{align}
