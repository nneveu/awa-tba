...
$s_3=19.4$~[m], see Fig.~\ref{awa-tba}. 
...
\begin{figure}
	...
	\caption{Pareto front comparing transverse beam size ($\sigma_{x,y}$) and transverse momentum ($\sigma_{px,py}$).}
	\label{fig:pareto1}
\end{figure}

\begin{figure}
	\includegraphics[width=0.5\textwidth]{../pareto_stat_plots/y_vs_py_pareto_front_quads_before_Q5}
	\caption{Pareto front comparing longitudinal beam size ($\sigma_z$) and energy spread ($dE$).}
	\label{fig:pareto2}
\end{figure}


As expected, the x dimension is impacted by the bending elements, and unable to maintain 
the small beam sizes seen in the y dimension. 
The $\sigma_x$ and $\sigma_{px}$ plots are of most interest, as these parameters are the heavily 
