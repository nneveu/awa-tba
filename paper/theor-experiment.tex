...
\lsedit{The next sentence is confusing.  If hyper volumes referring to fig. 6, why is there only one value?  Is it an average?}
...
\lsedit{The column descriptors for the table are not clear  - better formatting needed}
...
we chose this model to demonstrate the ability of the framework to tackle large problems.
...
the first four quadrupoles settings. 
...
\lsedit{Also, I had no idea what you meant by} %\text{rms}_{s}\left(s = s_1\right) until I saw the parameter list.   I would suggest`t' for time?  In any case maybe some text specifying what this is when you first mention it would be helpful.}
...
\lsedit{Why?  If all particles at lightspeed, then long beam size should not change in a dipole magnet}
...
\lsedit{the above sentence makes no sense to me}
...
\begin{figure}
	\includegraphics[width=0.5\textwidth]{../pareto_stat_plots/safehold/xy_vs_pxy_pareto_front_quads_before_Q5}
...
\begin{figure}
	\centering
	\includegraphics[width=0.48\textwidth]{../pareto_stat_plots/safehold/dE_vs_zrms_pareto_front_quads_before_Q5_zoomout}%
	\llap{\raisebox{1.9cm}{%  move next graphics to top right corner
			\includegraphics[height=4.5cm]{../pareto_stat_plots/safehold/dE_vs_zrms_pareto_front_quads_before_Q5}%
	}}
...
A select result is shown in Fig.~\ref{fig:stat}.  \lsedit{It would be a nice addition to say which point on the pareto front that was, perhaps marking it on the curve}
...
\lsedit{Some comments on Fig. 13: FIrst, is fig 13(a) necessary?  Why can't you just add a vertical scale to fig. 13b?  Also, how about adding more info to the figure caption; is s=0 the position of the cathode?  If so, put that in the caption.  You could describe what each curve is in the figure caption for completeness.  The bottom traces show the optimized beam sizes.. The top black trace shows.. including key elements such as the kicker location.  Why do you start showing the aperture at s=10?  Is it plenty big before that?  If so, in the figure caption you could say, the aperture is shown starting at s=10 because the aperture constraints are more acute after that point.}


\begin{figure}
	\includegraphics[width=0.48\textwidth]{../pareto_stat_plots/safehold/xy-max-min-zrms_newobj}
